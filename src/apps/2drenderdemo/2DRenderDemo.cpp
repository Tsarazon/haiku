 ...
 *
 * 2D Render Demo - bouncing shapes with collision
 ...
 *
 * Tracing focuses on KosmCG library performance and rendering correctness:
...
class RenderDemoView : public BView {
public:
	RenderDemoView()
		:
		BView(BRect(0, 0, 799, 599), "RenderDemoView", B_FOLLOW_ALL,
...
private:
	...
	void _Render()
	{
		...
		bigtime_t bgStart = system_time();
		{
			...
			kcg::Gradient::Stop stops[] = {
				{0.0f, kcg::Color::from_rgba8(45, 45, 70, 255)},
				{1.0f, kcg::Color::from_rgba8(10, 10, 18, 255)}
			};
		...
		for (int i = 0; i < kShapeCount; i++) {
			...
			switch (s.type) {
				...
				{
					ctx.set_shadow(kcg::Point(4, 4), 10,
					...
					kcg::Path circle = kcg::Path::Builder{}
						.add_circle(kcg::Point(0, 0), s.radius)
						.build();
					_T(T, "circle_setup");
					...
					ctx.draw_radial_gradient(grad,
						kcg::Point(-s.radius * 0.3f, -s.radius * 0.3f), 0,
						kcg::Point(-s.radius * 0.3f, -s.radius * 0.3f),
						s.radius * 1.2f,
						nullptr, kGradExtend);
				...
				case 1: // Rectangle - linear gradient + stroke
				{
					...
					kcg::Gradient::Stop stops[] = {
						{0.0f, kcg::Color::from_rgba8(s.r, s.g, s.b, 255)},
						{1.0f, kcg::Color::from_rgba8(
							s.r / 3, s.g / 3, s.b / 3, 255)}
					};
					kcg::Gradient grad = kcg::Gradient::create(stops);

					kcg::Path rect = kcg::Path::Builder{}
						.add_rect(kcg::Rect(-half, -half, size, size))
						.build();
					_T(T, "rect_setup");
					...
					ctx.draw_linear_gradient(grad,
						kcg::Point(-half, -half), kcg::Point(half, half),
						nullptr, kGradExtend);
					ctx.restore_state();
					_T(T, "rect_fill_grad");

					ctx.set_stroke_color(
						kcg::Color::from_rgba8(255, 255, 255, 160));
				...
				case 2: // Triangle - solid + shadow + rotation
				{
					...
					ctx.set_shadow(kcg::Point(3, 3), 8,
						kcg::Color(0, 0, 0, 0.45f));
					ctx.set_fill_color(
						kcg::Color::from_rgba8(s.r, s.g, s.b, 255));

					float rad = s.radius;
					kcg::Path tri = kcg::Path::Builder{}
						.move_to(0, -rad)
				...
				case 3: // Star - solid + dashed stroke + rotation
				{
					...
					kcg::Path::Builder builder;
					for (int k = 0; k < 5; k++) {
						float a1 = k * kcg::two_pi / 5.0f
							- kcg::half_pi;
						float a2 = a1 + kcg::pi / 5.0f;
						if (k == 0)
							builder.move_to(
					...
					ctx.set_line_cap(kcg::LineCap::Round);
					ctx.set_stroke_color(
						kcg::Color::from_rgba8(255, 255, 255, 180));
				...
				case 4: // Heart - bezier + gradient + opacity
				{
					...
					float rad = s.radius;
					kcg::Path heart = kcg::Path::Builder{}
						.move_to(0, rad * 0.7f)
						.cubic_to(
							-rad * 0.3f, rad * 0.3f,
							-rad, rad * 0.0f,
					...
					ctx.draw_radial_gradient(grad,
						kcg::Point(0, -s.radius * 0.4f), 0,
						kcg::Point(0, -s.radius * 0.2f),
						s.radius * 1.2f,
						nullptr, kGradExtend);
				...
				case 5: // Ellipse - linear gradient + rotation
				{
					...
					kcg::Gradient::Stop stops[] = {
						{0.0f, kcg::Color::from_rgba8(s.r, s.g, s.b, 255)},
						{0.5f, kcg::Color::from_rgba8(
							255, 255, 255, 180)},
						{1.0f, kcg::Color::from_rgba8(s.r, s.g, s.b, 255)}
					};
					kcg::Gradient grad = kcg::Gradient::create(stops);

					kcg::Path ellipse = kcg::Path::Builder{}
						.add_ellipse(kcg::Rect(
				...
				case 6: // Rounded rect - per-corner radii + shadow + stroke
				{
					...
					ctx.set_shadow(kcg::Point(5, 5), 12,
						kcg::Color(0, 0, 0, 0.5f));
					ctx.set_fill_color(
						kcg::Color::from_rgba8(s.r, s.g, s.b, 255));

					kcg::CornerRadii radii(
						size * 0.3f, size * 0.05f,
						size * 0.3f, size * 0.05f);
					kcg::Path rrect = kcg::Path::Builder{}
						.add_round_rect(
							kcg::Rect(-half, -half, size, size), radii)
						.build();
					_T(T, "rrect_setup");
					...
					ctx.clear_shadow();
					ctx.set_stroke_color(kcg::Color::from_rgba8(
						(uint8)(s.r * 0.5f), (uint8)(s.g * 0.5f),
						(uint8)(s.b * 0.5f), 255));
					ctx.set_line_width(3.0f);
					ctx.set_line_join(kcg::LineJoin::Round);
				...
				case 7: // Hexagon - conic gradient + rotation
				{
					...
					kcg::Gradient::Stop stops[] = {
						{0.0f, kcg::Color::from_rgba8(s.r, s.g, s.b, 255)},
						{0.33f, kcg::Color::from_rgba8(
							s.g, s.b, s.r, 255)},
						{0.66f, kcg::Color::from_rgba8(
							s.b, s.r, s.g, 255)},
						{1.0f, kcg::Color::from_rgba8(s.r, s.g, s.b, 255)}
					};
					kcg::Gradient grad = kcg::Gradient::create(stops);
					...
					kcg::Path::Builder builder;
					for (int k = 0; k < 6; k++) {
						float a = k * kcg::pi / 3.0f - kcg::half_pi;
						if (k == 0)
	...
	// Verify KosmCG actually rendered pixels at expected shape positions
	void _CheckPixels()
	{
		if (fBitmap == NULL)
			return;
	...
	void _OpenTrace()
	{
		...
		if (find_directory(B_DESKTOP_DIRECTORY, &desktop) != B_OK)
			return;
		BPath path(desktop);
		path.Append("2drender_trace.log");
		fTraceFile = fopen(path.Path(), "w");
		if (fTraceFile) {
			fprintf(fTraceFile,
				"# KosmCG performance trace (times in microseconds)\n"
				"# Shape features:\n");
			for (int i = 0; i < kShapeCount; i++) {
	...
	void _WriteSummary()
	{
		fprintf(fTraceFile,
			"\n"
			"========================================\n"
			"  KosmCG PERFORMANCE SUMMARY (%d frames)\n"
			"========================================\n",
			fFrameNum);
		...
		if (fPixelCheckFailed > 0) {
			fprintf(fTraceFile,
				"  *** KosmCG may have rendering bugs: "
...
class RenderDemoWindow : public BWindow {
public:
	RenderDemoWindow()
		:
		BWindow(BRect(100, 100, 899, 699), "2D Render Demo - Bouncing Shapes",
			B_TITLED_WINDOW, B_QUIT_ON_WINDOW_CLOSE)
	{
		AddChild(new RenderDemoView());
	}
};


class RenderDemoApp : public BApplication {
public:
	RenderDemoApp()
		:
		BApplication("application/x-vnd.Haiku-2DRenderDemo")
	{
	}

	virtual void ReadyToRun()
	{
		(new RenderDemoWindow())->Show();
	}
};
