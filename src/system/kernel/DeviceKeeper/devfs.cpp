...
//	#pragma mark - DeviceKeeper private API
...
//	#pragma mark - legacy driver stubs (removed in KosmOS)


extern "C" status_t
devfs_rescan_driver(const char* driverName)
{
	TRACE(("devfs_rescan_driver: %s (legacy, unsupported)\n", driverName));
	return B_UNSUPPORTED;
}
...
{
	// Legacy R5/Haiku driver publishing via device_hooks is no longer
	// supported. Drivers must use dk_keeper_info::publish_device()
	// with dk_device_ops instead.
	return B_UNSUPPORTED;
}
