...
extern dk_keeper_info gDeviceKeeperModule;
...
status_t
DMAResource::Init(dk_node* node, generic_size_t blockSize,
	uint32 bufferCount, uint32 bounceBufferCount)
{
	...
	uint32 value;
	if (gDeviceKeeperModule.get_property_uint32(node,
			KOSM_DMA_ALIGNMENT, &value, true) == B_OK)
		restrictions.alignment = (generic_size_t)value + 1;

	if (gDeviceKeeperModule.get_property_uint32(node,
			KOSM_DMA_BOUNDARY, &value, true) == B_OK)
		restrictions.boundary = (generic_size_t)value + 1;

	if (gDeviceKeeperModule.get_property_uint32(node,
			KOSM_DMA_MAX_SEGMENT_BLOCKS, &value, true) == B_OK)
		restrictions.max_segment_size = (generic_size_t)value * blockSize;

	if (gDeviceKeeperModule.get_property_uint32(node,
			KOSM_DMA_MAX_TRANSFER_BLOCKS, &value, true) == B_OK)
		restrictions.max_transfer_size = (generic_size_t)value * blockSize;

	if (gDeviceKeeperModule.get_property_uint32(node,
			KOSM_DMA_MAX_SEGMENT_COUNT, &value, true) == B_OK)
		restrictions.max_segment_count = value;

	uint64 value64;
	if (gDeviceKeeperModule.get_property_uint64(node,
			KOSM_DMA_LOW_ADDRESS, &value64, true) == B_OK) {
		restrictions.low_address = value64;
	}

	if (gDeviceKeeperModule.get_property_uint64(node,
			KOSM_DMA_HIGH_ADDRESS, &value64, true) == B_OK) {
