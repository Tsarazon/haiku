...
// AMD K8 C1E control bits (documented in BKDG)
...
// Intel CPUID leaf definitions (Intel SDM Vol. 2A)
constexpr uint32 CPUID_LEAF_BASIC_INFO = 0x01;
constexpr uint32 CPUID_LEAF_CACHE_PARAMS = 0x04;
constexpr uint32 CPUID_LEAF_POWER_MANAGEMENT = 0x06;
constexpr uint32 CPUID_LEAF_EXTENDED_FEATURES = 0x07;
constexpr uint32 CPUID_LEAF_EXTENDED_STATE = 0x0d;
constexpr uint32 CPUID_LEAF_FREQUENCY = 0x16;
constexpr uint32 CPUID_LEAF_HYBRID_INFO = 0x1a;
constexpr uint32 CPUID_LEAF_V2_TOPOLOGY = 0x1f;

// AMD extended CPUID leaf definitions (AMD APM Vol. 3)
constexpr uint32 CPUID_LEAF_AMD_EXTENDED = 0x80000001;
constexpr uint32 CPUID_LEAF_AMD_BRAND_1 = 0x80000002;
constexpr uint32 CPUID_LEAF_AMD_BRAND_2 = 0x80000003;
constexpr uint32 CPUID_LEAF_AMD_BRAND_3 = 0x80000004;
constexpr uint32 CPUID_LEAF_AMD_POWER = 0x80000007;
constexpr uint32 CPUID_LEAF_AMD_TOPOLOGY = 0x80000008;
constexpr uint32 CPUID_LEAF_AMD_CACHE_TOPOLOGY = 0x8000001d;

// Intel Model-specific constants (from Intel SDM)
constexpr uint32 INTEL_MODEL_DENVERTON = 0x5f;

// AMD Family-specific constants (from AMD BKDG)
constexpr uint32 AMD_FAMILY_10H = 0x10;
constexpr uint32 AMD_FAMILY_11H = 0x11;
constexpr uint32 AMD_FAMILY_17H = 0x17;
constexpr uint32 AMD_FAMILY_19H = 0x19;

// Pentium II/III/4 errata model thresholds
constexpr uint32 PENTIUM_III_MAX_MODEL = 13;
constexpr uint32 PENTIUM_4_MAX_MODEL = 6;

// Intel topology level types (CPUID.1Fh/0Bh.ECX[15:8])
constexpr int TOPOLOGY_LEVEL_TYPE_SMT = 1;
constexpr int TOPOLOGY_LEVEL_TYPE_CORE = 2;
...
static const FeatureFlag kCpuFeatures[] = {
	// CPUID.01h.EDX - Basic processor features
	...
	// CPUID.01h.ECX - Extended processor features
	...
	// CPUID.06h.EAX - Thermal and power management
	...
	// CPUID.07h.EBX - Structured extended features
	...
	// CPUID.0Dh.01h.EAX - Extended state features
...
constexpr size_t kFeatureCount = sizeof(kCpuFeatures) / sizeof(kCpuFeatures[0]);
...
// Check if CPU has broken PAT implementation
inline bool
has_broken_pat(const cpu_ent* cpu)
{
	...
	// Pentium II Errata A52, Pentium III Errata E27:
	// Upper PAT entries unusable as PAT bit ignored for 4K PTEs
	// Pentium 4 Errata N46: PAT bit can be assumed 0 in specific cases
...
{
	if (sUsePAT) {
		// When PAT is supported, ignore MTRRs (Intel SDM recommendation)
...
init_pat(int cpu)
{
	...
	// Use PAT entry 4 for write-combining (Intel SDM Vol. 3A, Section 11.12)
...
dump_feature_string(int currentCPU, cpu_ent* cpu)
{
	...
	// Table-driven feature enumeration
...
{
	// Try CPUID.1Fh (v2 topology) first, fall back to CPUID.0Bh
	uint32 leaf = 0;
	cpuid_info cpuid;
...
find_microcode_intel(addr_t data, size_t size, uint32 patchLevel)
{
	...
	while (size > 0) {
		...
		// Intel SDM requires 16-byte alignment
...
get_hybrid_cpu_type()
{
	...
	constexpr int X86_HYBRID_CPU_TYPE_ID_SHIFT = 24;
	cpuid_info cpuid;
...
detect_cpu(int currentCPU, bool full = true)
{
	...
	// Get CPU brand string (CPUID.80000002h-80000004h)
	...
	// Load feature bits (CPUID.1: EDX and ECX)
	get_current_cpuid(&cpuid, CPUID_LEAF_BASIC_INFO, 0);
	...
	// AMD extended features (CPUID.80000001h)
	if (maxExtendedLeaf >= CPUID_LEAF_AMD_EXTENDED) {
		get_current_cpuid(&cpuid, CPUID_LEAF_AMD_EXTENDED, 0);
		if (is_vendor_amd(cpu))
	...
	// Structured Extended Features (CPUID.7)
	...
	// Extended State Enumeration (CPUID.D)
	...
	// AMD Advanced Power Management (CPUID.80000007h)
	...
	// AMD Processor Topology (CPUID.80000008h)
	if (maxExtendedLeaf >= CPUID_LEAF_AMD_TOPOLOGY) {
		get_current_cpuid(&cpuid, CPUID_LEAF_AMD_TOPOLOGY, 0);
...
arch_cpu_preboot_init_percpu(kernel_args* args, int cpu)
{
	...
	// TSC synchronization for SMP systems
...
detect_amdc1e_noarat()
{
	...
	// AMD Family 12h+ support ARAT
	// AMD Family < 0Fh don't support C1E
	// AMD Family 0Fh with Model <= 40h don't support C1E
...
init_tsc_with_cpuid(kernel_args* args, uint32* conversionFactor)
{
	...
	// Denverton fallback: use base frequency
...
arch_cpu_init_percpu(kernel_args* args, int cpu)
{
	...
	// Make LFENCE dispatch-serializing on AMD 64-bit (AMD APM Vol. 2)
	if (is_vendor_amd(cpuEnt)) {
		uint32 family = compute_cpu_family(cpuEnt);
...
arch_cpu_init_post_vm(kernel_args* args)
{
	...
	// Enable XSAVE and extended processor state management
