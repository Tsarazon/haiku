...
class DeviceAddOn {
public:
	explicit DeviceAddOn(BInputServerDevice* device);
	~DeviceAddOn();
...
public:
	...
	status_t AddMethod();
	int32 Cookie() const { return fCookie; }
...
// Main InputServer class - maintain exact ABI compatibility
...
private:
	...
	void _ProcessMouseEvent(BMessage* event);
	bool _ProcessKeyEvent(BMessage* event, EventList& events, int32 index);
