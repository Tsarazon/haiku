...
#define OHCI_PCI_DEVICE_MODULE_NAME "busses/usb/ohci/pci/dk_driver_v1"
...
static status_t
ohci_attach(dk_node* node, void** device_cookie)
{
	CALLED();
	...
	bus->ohci = new(std::nothrow) OHCI(&bus->pciinfo, bus->pci, bus->device,
		stack, node);
	if (bus->ohci == NULL) {
		free(bus);
		return B_NO_MEMORY;
...
static void
ohci_detach(void* device_cookie)
{
	CALLED();
...
static const dk_match_rule sOhciPciMatchRules[] = {
	...
	{ KOSM_DEVICE_INTERFACE,  B_UINT16_TYPE, { .ui16 = PCI_usb_ohci } },
	{}
};

static const dk_match_dict sOhciPciMatchDict = {
	sOhciPciMatchRules, 0
};


// Root device that binds to the PCI bus.
static dk_driver_info sOHCIDevice = {
	{
...
OHCI::OHCI(pci_info *info, pci_device_ops* pci, pci_device* device, Stack *stack,
	dk_node* node)
