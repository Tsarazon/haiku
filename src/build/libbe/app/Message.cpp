...
#define DEBUG_FUNCTION_ENTER	\
	debug_printf("msg thread: %ld; this: %p; header: %p; fields: %p;" \
		...
		find_thread(nullptr), this, fHeader, fFields, fData, what, reinterpret_cast<char *>(&what), \
...
{
	Type *item = reinterpret_cast<Type *>(pointer);
	item->PrintToStream();
}
	...
	bool deep) const
{
	...
	for (uint32 i = 0; i < fHeader->field_count; i++) {
		...
		const char *name = reinterpret_cast<const char *>(fData + field->offset);
		if (ignoreFieldOrder) {
			...
			const char *otherName = reinterpret_cast<const char *>(other.fData)
				+ otherField->offset;
...
BMessage::_PrintToStream(const char* indent) const
{
	...
	printf("BMessage(");
	if (isprint(*reinterpret_cast<char *>(&value)))
		printf("'%.4s'", reinterpret_cast<char *>(&value));
	else
...
BMessage::Rename(const char *oldEntry, const char *newEntry)
{
	...
	auto *nextField = &fHeader->hash_table[hash];
...
BMessage::Flatten(BDataIO *stream, ssize_t *size) const
{
	...
	auto result1 = stream->Write(fHeader, sizeof(message_header));
...
BMessage::Unflatten(BDataIO *stream)
{
	...
	uint8 *header = reinterpret_cast<uint8 *>(fHeader);
...
BMessage::_ResizeData(uint32 offset, int32 change)
{
	...
	if (change > 0) {
		...
		uint8 *newData = static_cast<uint8 *>(realloc(fData, size));
	...
	field_header **result)
{
	...
	if (fFieldsAvailable <= 0) {
		...
		field_header *newFields = static_cast<field_header *>(realloc(fFields,
...
BMessage::_RemoveField(field_header *field)
{
	...
	if (fFieldsAvailable > MAX_FIELD_PREALLOCATION) {
		...
		field_header *newFields = static_cast<field_header *>(realloc(fFields, size));
...
BMessage::RemoveData(const char *name, int32 index)
{
	...
	if ((field->flags & FIELD_FLAG_FIXED_SIZE) != 0) {
		...
		size_t currentSize = *reinterpret_cast<uint32 *>(pointer) + sizeof(uint32);
	...
	const void *data, ssize_t numBytes)
{
	...
	if ((field->flags & FIELD_FLAG_FIXED_SIZE) != 0) {
		...
		size_t currentSize = *reinterpret_cast<uint32 *>(pointer);
...
BMessage::FindRef(const char *name, int32 index, entry_ref *ref) const
{
	...
	if (error == B_OK)
		error = BPrivate::entry_ref_unflatten(ref, reinterpret_cast<char *>(data), size);
	else
		*ref = entry_ref();
