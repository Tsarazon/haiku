--[[
    addattr - Add attributes to files
    Mirrors: src/tools/addattr/Jamfile

    Note: Requires HOST_LIBBE (libbe built for host platform).
    Will be buildable once src/build/libbe is ported to xmake.
]]
...
-- Source from src/bin/addattr/
local bin_addattr_src = path.join(haiku_top, "src", "bin", "addattr")
...
target("addattr")
    ...
    -- Sources from src/bin/addattr/
    add_files(path.join(bin_addattr_src, "main.cpp"))
    add_files(path.join(bin_addattr_src, "addAttr.cpp"))
    ...
    -- For now this target will fail to link without HOST_LIBBE
target_end()
