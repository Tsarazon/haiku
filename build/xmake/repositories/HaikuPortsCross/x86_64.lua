...
-- Stage 0 packages for secondary x86 architecture
STAGE0_SECONDARY_X86 = {
    "gcc_bootstrap_x86-8.3.0_2019_05_24-1",
    "gcc_bootstrap_x86_syslibs-8.3.0_2019_05_24-1",
    "gcc_bootstrap_x86_syslibs_devel-8.3.0_2019_05_24-1",
}
...
-- Stage 1 packages for secondary x86 architecture
STAGE1_SECONDARY_X86 = {
    "binutils_bootstrap_x86-2.28.1_2017_08_05-1",
    "expat_bootstrap_x86-2.1.0-1",
...
local function get_all_arch_packages(include_secondary)
    local packages = {}
    ...
    -- Add secondary x86 stage 0 if requested
    if include_secondary then
        for _, pkg in ipairs(STAGE0_SECONDARY_X86) do
            table.insert(packages, pkg)
        end
    end
    ...
    -- Add secondary x86 stage 1 if requested
    if include_secondary then
        for _, pkg in ipairs(STAGE1_SECONDARY_X86) do
            table.insert(packages, pkg)
        end
    end
...
local function build_version_lookup()
    local lookup = {}
    local all_packages = get_all_arch_packages(true)
...
return {
    ...
    STAGE0_SECONDARY_X86 = STAGE0_SECONDARY_X86,
    STAGE1_PACKAGES = STAGE1_PACKAGES,
    STAGE1_SECONDARY_X86 = STAGE1_SECONDARY_X86,
